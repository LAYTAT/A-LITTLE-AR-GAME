...
public class PlayerMoveAnimationController : MonoBehaviour, IDragHandler, IPointerDownHandler, IEndDragHandler
{
    ...
    void Update()
    {
        ...
        target.Set(Vx * Time.deltaTime, 0,
            (Vy > 0.7f ? 4f : (Vy > 0f ? 1.5f : 1f)) * Vy * Time.deltaTime);
        motion.Move(motion.transform.TransformDirection(target));
        action.SetFloat("H", Vx);
        action.SetFloat("V", Vy);
    }
    ...
    {
        dx = eventData.position.x - x0;
        dy = eventData.position.y - y0;
        float s = dx * dx + dy * dy;
        if (s <= 10000f)
            offset.Set(dx, dy, 0f);
        else
